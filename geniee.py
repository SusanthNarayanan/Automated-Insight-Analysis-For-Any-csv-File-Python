...
file = input("Kindly enter the file location: ")
df = pd.read_csv(file)
...
def analysis():
    ...
    if (ch == 1):
        ...
        sn.heatmap(corr, linewidths=0.1, square=True,
                   annot=True, cmap='GnBu', linecolor='black')
        ...
        cor = df.corr().iloc[:, -
                             1].sort_values(ascending=False)[1:].to_frame()
        sn.set(font_scale=1.6)
        plt.figure(figsize=(12, 8))
        sn.heatmap(data=cor, cmap="jet", center=0.1, annot=True,
                   vmax=.5, linewidths=0.1, annot_kws={"size": 16})
        plt.show()
        analysis()
    elif (ch == 5):
